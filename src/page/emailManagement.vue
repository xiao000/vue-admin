...
<template>
  <div class="emailManagement">
      ...
      <el-table :data="tableData" style="width: 100%">
        <el-table-column prop="createDate" label="订阅时间"></el-table-column>
...
export default {
  ...
  methods: {
    getAPI(emailVal,currentVal,pageSizeVal){
      var that=this;
       this.$axios
            .post("/admin/subscribe/page", {
              email: emailVal,
              current:currentVal,
    ...
    onReset(formName) {
      ...
      this.form.email=""
      this.getAPI(this.form.email,this.currentPage,this.pageSize);
    },
    // 分页
    handleSizeChange(val) {
      // console.log(`每页 ${val} 条`);
       this.getAPI(this.form.email,this.currentPage,val);
    },
    handleCurrentChange(val) {
      // console.log(`当前页: ${val}`);
        this.getAPI(this.form.email,val,this.pageSize);
    }
  }
};
</script>
